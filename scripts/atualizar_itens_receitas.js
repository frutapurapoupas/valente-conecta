// ============================================
// SCRIPT PARA ATUALIZAR INGREDIENTES DAS RECEITAS
// Execute: node scripts/atualizar_itens_receitas.js
// ============================================

// Mapeamento de ingredientes por receita (com quantidades em kg)
...
async function atualizarIngredientes() {
  console.log('\n🔧 ATUALIZANDO INGREDIENTES DAS RECEITAS...\n');
  
  // Buscar todos os ingredientes
  const ingredientesResponse = await fetch('http://localhost:3000/api/admin/ingredients');
  const ingredientes = await ingredientesResponse.json();
  
  // Criar mapa nome -> id
  const ingredientesMap = {};
  ingredientes.forEach(ing => {
    ingredientesMap[ing.name] = ing.id;
  });
  
  // Buscar todas as receitas
  const receitasResponse = await fetch('http://localhost:3000/api/admin/recipes');
  const receitas = await receitasResponse.json();
  
  let atualizadas = 0;
  
  for (const receita of receitas) {
    const itemsToAdd = recipeIngredients[receita.name];
    ...
    if (itemsToAdd) {
      ...
      for (const item of itemsToAdd) {
        const ingId = ingredientesMap[item.nome];
        if (ingId) {
          items.push({
            ingredientId: ingId,
          ...
          missingIngredients.push(item.nome);
        }
      }
      
      if (items.length > 0) {
        // Atualizar receita com os ingredientes
        const updateBody = {
          id: receita.id,
          name: receita.name,
        ...
        if (response.ok) {
          ...
          for (const item of items) {
            ...
            if (ing) {
              custoTotal += item.quantity * ing.currentPrice;
            }
          }
          ...
          if (missingIngredients.length > 0) {
            console.log(`   ⚠️ Faltantes: ${missingIngredients.join(', ')}`);
          }
        } else {
          console.log(`❌ Erro ao atualizar: ${receita.name}`);
        }
      }
    }
  }
