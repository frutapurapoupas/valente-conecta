...
export default function CatalogoMarmita() {
  ...
  // Busca os pratos reais do banco de dados na inicializaÃ§Ã£o
  useEffect(() => {
  ...
  return (
    <div className="container mx-auto p-6">
      <h1 className="text-2xl font-bold mb-6">CardÃ¡pio Chef Neide</h1>
      ...
      {cartItems.length > 0 && (
        <div className="fixed bottom-6 right-6 bg-blue-600 text-white p-4 rounded-full shadow-2xl cursor-pointer hover:bg-blue-700 transition" onClick={() => setIsModalOpen(true)}>
          <span className="font-bold">ðŸ›’ Ver Carrinho (R$ {total.toFixed(2)})</span>
        </div>
      )}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white p-6 rounded-xl max-w-md w-full shadow-2xl">
            ...
            <h3 className="font-bold mb-2 text-sm text-gray-700">Escolha a Forma de Pagamento (SimulaÃ§Ã£o):</h3>
            <div className="grid grid-cols-2 gap-2 mb-6">
              <button className="border p-3 rounded-lg font-medium hover:bg-gray-50 active:border-blue-500" onClick={() => alert('SimulaÃ§Ã£o: Pix Selecionado!')}>ðŸ“± PIX</button>
              <button className="border p-3 rounded-lg font-medium hover:bg-gray-50 active:border-blue-500" onClick={() => alert('SimulaÃ§Ã£o: CartÃ£o Selecionado!')}>ðŸ’³ CartÃ£o</button>
            </div>
            ...
            <div className="flex gap-2">
              ...
              <button className="w-1/2 bg-blue-600 text-white py-2 rounded-lg font-medium hover:bg-blue-700" onClick={() => { alert('Pedido ConcluÃ­do e enviado para a produÃ§Ã£o!'); setCartItems([]); setIsModalOpen(false); }}>Finalizar</button>
            </div>
          </div>
        </div>
      )}
