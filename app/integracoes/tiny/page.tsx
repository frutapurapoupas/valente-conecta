...
export default function IntegracaoTiny() {
  ...
  const carregarConfig = () => {
    try {
      const salvo = localStorage.getItem('tiny_config')
      if (salvo) {
  ...
  const salvarConfig = () => {
    try {
      localStorage.setItem('tiny_config', JSON.stringify(config))
  ...
  const testarConexao = async () => {
    if (!config.apiKey || !config.idEmpresa) {
      alert('Preencha API Key e ID da Empresa')
      return
    }
    ...
    try {
      // Simular teste de conexão com API do Tiny
      const response = await fetch(`https://api.tiny.com.br/api2/produtos.pesquisa.php?token=${config.apiKey}&pesquisa=&pagina=1`)
      
      if (response.ok) {
        const data = await response.json()
        if (data.retorno && data.retorno.status === 'OK' && data.retorno.produtos) {
          setConfig(prev => ({
            ...prev,
            ativo: true,
          ...
          alert('Conexão com Tiny ERP estabelecida com sucesso!')
        } else {
      ...
      alert('Erro ao conectar com Tiny ERP. Verifique suas credenciais.')
    } finally {
      setLoading(false)
      setTestando(false)
    }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-4">
              ...
              <h1 className="text-xl font-semibold text-gray-900">Integração Tiny ERP</h1>
            </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="space-y-4">
                {[
                  { numero: 1, titulo: 'Configurar API', descricao: 'Obter token de acesso', concluido: !!config.apiKey },
                  { numero: 2, titulo: 'Identificar Empresa', descricao: 'Localizar ID da empresa', concluido: !!config.idEmpresa },
            ...
            <div className="bg-white rounded-lg shadow p-8">
              {passo === 1 && (
                <div className="space-y-6">
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900 mb-4">Passo 1: Configurar API Tiny</h3>
                    <p className="text-gray-600 mb-6">
                      Para começar, você precisa obter seu token de API no painel do Tiny ERP.
                    </p>
                  </div>
              ...
              {passo === 2 && (
                <div className="space-y-6">
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900 mb-4">Passo 2: Identificar Empresa</h3>
                    <p className="text-gray-600 mb-6">
                      Agora precisamos identificar qual empresa do Tiny ERP será sincronizada.
                    </p>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <input
                        type="text"
                        value={config.idEmpresa}
                        onChange={(e) => setConfig({...config, idEmpresa: e.target.value})}
                    ...
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                      <h4 className="font-medium text-yellow-900 mb-2">Onde encontrar este ID?</h4>
                      <ol className="list-decimal list-inside space-y-2 text-sm text-yellow-800">
                        <li>No painel do Tiny ERP, vá em "Configurações"</li>
                        <li>Procure por "Dados da Empresa"</li>
                        <li>O ID está visível na página de configurações</li>
                        <li>Geralmente é um número de 8 dígitos</li>
                        <li>Também pode ser encontrado na URL do painel</li>
                      </ol>
                    </div>
                  </div>
              ...
              {passo === 3 && (
                <div className="space-y-6">
                  <div>
                    ...
                    <p className="text-gray-600 mb-6">
                      Vamos validar se a conexão com o Tiny ERP está funcionando corretamente.
                    </p>
                  </div>
                  ...
                  <div className="space-y-4">
                      ...
                      <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex items-center">
                            ...
                            <span className={`ml-2 text-sm font-medium ${
                              config.idEmpresa ? 'text-green-600' : 'text-red-600'
                            }`}>
                              {config.idEmpresa ? 'Configurada ✓' : 'Não configurada ✗'}
                            </span>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                      ...
                      <ol className="list-decimal list-inside space-y-2 text-sm text-green-800">
                        ...
                        <li>Seus dados do Tiny ERP estarão integrados ao sistema</li>
                      </ol>
                    </div>
                  </div>
