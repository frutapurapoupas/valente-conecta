...
import { Gift, ArrowRight, CheckCircle, Smartphone, Download, Share2 } from "lucide-react";

export default function ConviteAdminPage() {
  const router = useRouter();
  const { user } = useApp();
  const [deferredPrompt, setDeferredPrompt] = useState<any>(null);
  const [baseUrl, setBaseUrl] = useState("");
  const [isClient, setIsClient] = useState(false);
  const codigo = "ADMIN_VALENTE_2026";
  const convidadoPor = "Admin Master";
  ...
  const handleCompartilhar = async () => {
    const link = `${baseUrl}/convite/admin`;
    if (navigator.share) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900/20 to-slate-900 text-white pb-20">
      <header className="bg-gradient-to-r from-green-400 to-green-700 p-4 text-center sticky top-0 z-40">
        <h1 className="text-white font-bold text-lg">Convite Admin Master</h1>
      </header>
      ...
      <main className="max-w-2xl mx-auto p-6 space-y-6">
        <div className="text-center">
          ...
          <p className="text-gray-400">{convidadoPor} te convidou para experimentar o Valente Conecta</p>
        </div>

        <div className="bg-white/10 rounded-2xl p-6 space-y-4">
          <h3 className="font-bold text-white text-lg">🎁 Benefícios exclusivos:</h3>
          <div className="space-y-3">
            <div className="flex items-center gap-3"><CheckCircle className="w-5 h-5 text-green-400" /><span>R$5 de bônus na carteira</span></div>
            <div className="flex items-center gap-3"><CheckCircle className="w-5 h-5 text-green-400" /><span>Acesso completo a todos os serviços</span></div>
            <div className="flex items-center gap-3"><Smartphone className="w-5 h-5 text-blue-400" /><span>App instalável na tela inicial</span></div>
          </div>
        </div>

        <div className="bg-white/10 rounded-2xl p-4">
          <p className="text-gray-400 text-sm mb-2">📋 Código de convite</p>
