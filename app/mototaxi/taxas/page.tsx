...
// Caminho: C:\valente_conecta\app\mototaxi\taxas\page.tsx
//
// Lista as taxas de uso do Moto Taxi (cliente ou motorista) do usuario
// logado, com botao pra pagar via Mercado Pago. E' pra onde o lembrete
// automatico (push, ver lib/mototaxi/taxaUso.ts) leva quando toca na
// notificacao.
...
interface TaxaUso {
  ...
  created_at: string;
  mototaxi_corridas?: { origem: string; destino: string; created_at: string } | null;
}

export default function MototaxiTaxasPage() {
  ...
  const carregar = async () => {
    ...
    try {
      ...
      const resp = await fetch(`/api/mototaxi/taxas?usuarioId=${usuarioId}`, { cache: "no-store" }).then((r) => r.json());
  ...
  const pagar = async (taxaId: string) => {
    setPagando(taxaId);
    try {
      const resp = await fetch(`/api/mototaxi/taxas/${taxaId}/pagar`, { method: "POST" }).then((r) => r.json());
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 sm:p-6">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h1 className="text-lg font-bold text-gray-800">Taxas de uso — Moto Táxi</h1>
      </div>
      <p className="text-sm text-gray-600 mb-5">Taxa cobrada pela plataforma em cada corrida concluída pelo app. Com um plano pago, essa taxa não é cobrada.</p>
                  ...
                  >
                    <div className="min-w-0">
                      <p className="font-semibold text-gray-800">
                        {taxa.papel === "cliente" ? "Taxa como passageiro" : "Taxa como motorista"}
                      </p>
                      {taxa.mototaxi_corridas && (
                        <p className="text-sm text-gray-600 truncate">{taxa.mototaxi_corridas.origem} → {taxa.mototaxi_corridas.destino}</p>
                      )}
