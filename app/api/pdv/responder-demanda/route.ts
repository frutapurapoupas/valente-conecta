...
//
// As fotos (produto e codigo de barras) ja chegam como URL: o upload em si
// acontece antes, no client, via MidiaUploader (Supabase Storage direto) —
// essa rota so' grava referencia, e' rapida.
//
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.fotoCodigoBarrasUrl && !produto.foto_codigo_barras_url) {
      const supabase = createClient();
      await supabase
        .from('pdv_produtos_catalogo')
        .update({ foto_codigo_barras_url: body.fotoCodigoBarrasUrl, updated_at: new Date().toISOString() })
        .eq('id', produto.id)
        .is('foto_codigo_barras_url', null); // so' preenche se ninguem preencheu entre o find e agora
    }
