// Caminho: C:\valente_conecta\app\api\mototaxi\taxas\[id]\pagar\route.ts
//
// Cria a preferencia Mercado Pago pra pagar uma taxa de uso pendente (ver
// lib/mototaxi/taxaUso.ts). Mesmo padrao ja usado em
// app/api/carona/desbloqueios/route.ts: cria/atualiza o registro primeiro,
// depois gera o checkout; o webhook confirma (prefixo "mototaxi_taxa_" em
...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const label = taxa.papel === 'cliente' ? 'passageiro' : 'motorista';
    const preferencePayload = {
      external_reference: `mototaxi_taxa_${taxa.id}`,
      notification_url: notificationUrl,
      items: [
        {
          id: taxa.id,
          title: `Moto Táxi — taxa de uso do app (${label})`,
          quantity: 1,
          currency_id: 'BRL',
      ...
      metadata: { taxaId: taxa.id, origem: 'mototaxi_taxa_uso' },
    };
    ...
    await supabase.from('mototaxi_taxas_uso').update({ mp_preference_id: mpData.id, updated_at: new Date().toISOString() }).eq('id', taxa.id);
