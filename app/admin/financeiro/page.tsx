    ...
    <div className="min-h-screen bg-black text-white antialiased font-sans overflow-x-hidden">
      ...
      <div className="origin-top-left scale-[0.55] w-[181.8%] p-10">
        <header className="flex justify-between items-end mb-12 border-b-4 border-zinc-900 pb-10 no-print">
          ...
          <button onClick={() => { setIsEditing(false); setShowModal(true); }} className="bg-red-600 text-white px-16 py-8 rounded-25 text-5xl font-black uppercase italic shadow-[0_0_50px_rgba(220,38,38,0.4)] hover:bg-white hover:text-red-600 transition-all">+ Novo Lançamento</button>
        </header>

        {/* FILTROS DESIGN INDUSTRIAL */}
        ...
        <section className="bg-white text-black p-12 rounded-60 shadow-2xl print-area">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b-4 border-zinc-200 text-zinc-400 font-black uppercase text-2xl italic text-center">
                ...
                <th className="p-4 text-left">Descrição / Fornecedor / Obs</th>
          ...
          {despesas.length === 0 && <div className="text-center py-20 text-4xl font-black text-zinc-200 uppercase italic">Nenhum registro para este período.</div>}
        </section>
      </div>
            ...
            <div className="grid grid-cols-2 gap-12">
              <div className="space-y-8">
                <div>
                  ...
                  <input value={form.nome} onChange={e => setForm({...form, nome: e.target.value})} className="w-full bg-zinc-50 border-4 border-zinc-200 p-8 rounded-30 text-4xl font-black uppercase outline-none" />
              ...
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <input value={form.forn} onChange={e => setForm({...form, forn: e.target.value})} className="w-full bg-zinc-50 border-4 border-zinc-200 p-6 rounded-20 text-3xl font-black uppercase outline-none" />
                  </div>
                </div>
              </div>
            </div>
            <div className="mt-16 flex gap-6">
              <button onClick={() => setShowModal(false)} className="flex-1 bg-zinc-100 p-10 rounded-40 text-4xl font-black uppercase italic text-zinc-400">Cancelar</button>
