...
export default function AdminClassificadosPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("realtime-admin-classificados")
      .on(
        "postgres_changes",
  ...
  async function salvar(e: FormEvent) {
    ...
    if (form.id) {
      await supabase.from("classificados").update(payload).eq("id", form.id);
    } else {
  ...
  async function excluir(id: string) {
    const ok = window.confirm("Deseja excluir este classificado?");
    if (!ok) return;
  ...
  function editar(item: Classificado) {
    setForm({
      id: item.id,
      titulo: item.titulo || "",
  ...
  return (
    <AppShell
      title="Admin • Classificados"
      subtitle="Gerencie anúncios com reflexo imediato na área pública."
    >
      <form onSubmit={salvar} className="card">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="section-title">
            {form.id ? "Editar classificado" : "Novo classificado"}
          </h2>
      ...
      <div className="mt-4">
        {loading ? (
          ...
          <div className="card">Nenhum classificado cadastrado.</div>
        ) : (
