...
import { Search, Plus, Edit, Trash2, CheckCircle, XCircle, Clock } from 'lucide-react';
...
export default function ContasPagarPage() {
  ...
  const carregarContas = () => {
    ...
    if (stored) {
      ...
      const contasFiltradas = (dados.transacoes || []).filter((t: Transacao) => t.tipo === 'despesa' && t.status !== 'pago');
  ...
  const salvarConta = (e: React.FormEvent) => {
    ...
    carregarContas();
    setShowModal(false);
    setFormData({ descricao: '', categoria: 'Contas', valor: 0, data: new Date().toISOString().split('T')[0], recorrente: false });
  };
          ...
          contas.map((conta) => (
            <div key={conta.id} className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow flex justify-between items-center">
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-400 dark:text-gray-500">Vence em: {new Date(conta.data).toLocaleDateString()}</p>
              </div>
              <div className="text-right">
                <p className="text-xl font-bold text-red-600">{formatCurrency(conta.valor)}</p>
                <div className="flex gap-2 mt-2">
                  <button onClick={() => marcarComoPago(conta.id)} className="text-green-600 hover:text-green-800" title="Marcar como pago"><CheckCircle className="w-5 h-5" /></button>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md">
            ...
            <form onSubmit={salvarConta} className="space-y-4">
              ...
              <div className="flex justify-end gap-3"><button type="button" onClick={() => setShowModal(false)} className="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700">Cancelar</button><button type="submit" className="px-4 py-2 bg-red-600 text-white rounded-lg">Salvar</button></div>
            </form>
          </div>
        </div>
      )}
