...
export default function AdminMototaxiPage() {
  ...
  const [motoristas, setMotoristas] = useState([
    ...
    { id: 3, nome: "JosÃ© Roberto", veiculo: "Suzuki Intruder", status: "inativo", corridasHoje: 0 },
  ]);
  ...
  return (
    <div className="min-h-screen bg-gray-900 pb-20">
      <header className="gradient-primary p-4 flex items-center gap-3">
        ...
        <h1 className="text-white font-bold text-xl">ðŸ›µ Admin Moto TÃ¡xi</h1>
      </header>
      ...
      <div className="p-4">
        <h2 className="text-white font-bold mb-3">ðŸï¸ Motoristas</h2>
          ...
          {motoristas.map(m => (
            <div key={m.id} className="bg-white/10 rounded-2xl p-3 flex items-center justify-between">
              ...
              <span className={`px-2 py-1 rounded-full text-xs ${m.status === "ativo" ? "bg-green-500" : "bg-gray-600"}`}>
                {m.status === "ativo" ? "âœ… Ativo" : "âŒ Inativo"}
              </span>
            </div>
          ))}
        </div>
