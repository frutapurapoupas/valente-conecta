...
export default function HomePage() {
  ...
  const [modalSolicitacao, setModalSolicitacao] = useState<{ open: boolean; servico: string; categoria: string }>({ 
    open: false, 
    servico: "", 
    categoria: "" 
  });
  ...
  useEffect(() => {
    ...
    if (alreadyInstalled || alreadyDismissed || isStandalone || isInstalledApp) {
      setShowInstallBanner(false);
    }
    
    // Se não tem suporte a PWA
  ...
  const handleInstall = async () => {
    ...
    // Tentativa 1: Usar beforeinstallprompt (instalação nativa)
    if (deferredPrompt) {
      try {
        ...
        if (outcome === "accepted") {
          ...
          setShowInstallBanner(false);
          toast.success("✅ App instalado com sucesso na tela inicial!");
        } else {
          toast.info("Instalação cancelada. Clique em 'Instalar' novamente quando quiser.");
        }
        setDeferredPrompt(null);
      } catch (error) {
        console.error("Erro na instalação:", error);
        toast.error("Não foi possível instalar automaticamente. Tente novamente.");
      }
    } 
    // Tentativa 2: Usar navigator.install (Android)
    else if (navigator && 'install' in navigator) {
      try {
        ...
        toast.error("Instalação não suportada neste navegador.");
      }
    }
    // Tentativa 3: Guia de instalação manual (apenas se não houver suporte)
    else {
      toast.error(
        (t) => (
          <div className="text-center">
            <p className="font-bold mb-2">📱 Instalação</p>
            <p className="text-sm mb-2">Seu navegador não suporta instalação automática.</p>
            <p className="text-sm">Acesse pelo Chrome ou Samsung Internet.</p>
            <button 
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div ref={homeRef} className="min-h-screen">
        <div className="bg-gradient-to-r from-green-400 to-green-700 px-5 pt-8 pb-5 rounded-b-3xl">
          <div className="flex justify-between items-center mb-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <button 
                ...
                className="w-8 h-8 bg-white/30 rounded-full flex items-center justify-center hover:bg-white/40 transition"
                aria-label="Perfil"
              >
          ...
          <div className="mb-3">
            <div className="flex items-center bg-white rounded-2xl px-3 py-2 shadow-lg">
              ...
              <button 
                ...
                className="text-gray-400 mr-2 hover:text-green-500 transition"
              ...
              <button 
                ...
                className="bg-green-500 text-white px-3 py-1 rounded-xl text-xs font-medium hover:bg-green-600 transition"
              >
                Buscar
              </button>
            </div>
          ...
          <div className="bg-white/20 rounded-2xl p-3">
            ...
            <div className="flex gap-2 mt-2">
              <button className="flex-1 bg-white text-green-600 py-1 rounded-xl font-semibold text-xs hover:bg-gray-100 transition">
                Receber
              </button>
              <button className="flex-1 bg-white/30 text-white py-1 rounded-xl font-semibold text-xs hover:bg-white/40 transition">
                Pagar
              </button>
            </div>
          </div>
        ...
        {/* BANNER DE INSTALAÇÃO PWA - PRIMEIRA TELA */}
              ...
              <div className="p-3 flex items-center justify-between">
                <div className="flex items-center gap-3 flex-1">
                  ...
                  <div>
                    ...
                    <p className="text-white/70 text-xs">Acesso rápido pela tela inicial</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="bg-white text-blue-600 px-4 py-2 rounded-xl font-bold text-sm hover:bg-gray-100 transition flex items-center gap-1 disabled:opacity-50 shadow-md"
                  >
                    {isInstalling ? (
        ...
        <div className="px-5 mb-6">
          <div 
            onClick={showQR} 
            className="bg-gradient-to-r from-yellow-400 to-amber-500 rounded-2xl p-4 flex items-center justify-between cursor-pointer shadow-lg hover:shadow-xl transition transform hover:scale-[1.02]"
          >
            <div>
            ...
            <div className="bg-gradient-to-r from-gray-800 to-gray-900 rounded-2xl shadow-xl border border-yellow-500/30 overflow-hidden">
              ...
              <div className="p-3 space-y-2 max-h-48 overflow-auto">
                {notificacoesAdmin.map((notif) => (
                  <div key={notif.id} className="flex items-start gap-2 p-2 rounded-lg bg-white/5">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-gray-500 text-[10px] mt-0.5">{notif.data}</p>
                    </div>
      ...
      {/* BLOCOS DE CATEGORIAS (MANTIDOS IGUAIS) */}
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden">
            <div className="flex justify-between items-center p-3 border-b border-gray-100">
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-700 font-bold text-sm">Suas Notificações</span>
                <span className="bg-red-500 text-white text-xs px-2 py-0.5 rounded-full">
                  {notificacoesNaoLidas.length}
                </span>
              </div>
              <button 
                onClick={() => setNotificacoesUsuario(prev => prev.map(n => ({ ...n, lida: true })))} 
            ...
            <div className="p-3 space-y-2 max-h-48 overflow-auto">
              {notificacoesUsuario.filter(n => !n.lida).map((notif) => (
                <div 
                  key={notif.id} 
                  onClick={() => marcarNotificacaoLida(notif.id)} 
                  className="bg-gray-50 rounded-xl p-2 flex items-center gap-2 cursor-pointer hover:bg-gray-100 transition"
                >
                  <div className={`w-2 h-2 rounded-full ${
                    notif.tipo === "bonus" ? "bg-green-500" : 
                    notif.tipo === "pedido" ? "bg-orange-500" : "bg-purple-500"
                  }`}></div>
                  <p className="text-gray-600 text-sm flex-1">{notif.mensagem}</p>
      ...
      <SolicitacaoModal
        ...
        onClose={() => setModalSolicitacao({ open: false, servico: "", categoria: "" })}
