    ...
    <div className="min-h-screen bg-gray-900 text-white p-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
          <div>
            ...
            <h1 className="text-2xl font-bold flex items-center gap-2 mt-1">
              <Eye className="text-green-400" /> Preview Cardápio
            </h1>
            <p className="text-sm text-gray-400">{pratosAtivos.length} pratos disponíveis</p>
          </div>
        ...
        <div className="bg-gray-800/30 rounded-xl border border-gray-700 p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold flex items-center gap-2">
              <Utensils className="text-green-400" /> Cardápio {diaSelecionado !== 'todos' ? `- ${diaSelecionado}` : 'da Semana'}
            </h2>
          ...
          <div className="space-y-6">
            {pratosAtivos.length === 0 ? (
              <div className="text-center py-12 text-gray-400">
                ...
                <p>Nenhum prato disponível para este dia</p>
              </div>
            ) : (
              diasSemana.map(dia => {
        ...
        <div className="mt-4 p-4 bg-gray-800/20 rounded-lg border border-gray-700 text-xs text-gray-500">
          <p>💡 Este é o cardápio que os clientes veem. Para alterar, vá em "Gerenciar Pratos".</p>
          <p className="mt-1">📱 Os clientes acessam em: /cozinha</p>
        </div>
      </div>
    </div>
  );
