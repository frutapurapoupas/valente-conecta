// ============================================
// SCRIPT CORRIGIDO PARA ATUALIZAR RECEITAS
// Execute: node scripts/corrigir_receitas.js
// ============================================
...
async function corrigirReceitas() {
  ...
  for (const receita of receitas) {
    const ingredientesLista = recipeIngredients[receita.name];
    
    if (!ingredientesLista) {
      console.log(`⚠️ Receita não encontrada no mapa: ${receita.name}`);
      continue;
    }
    ...
    for (const item of ingredientesLista) {
      const ing = ingMap[item.nome];
      if (ing) {
        const itemCusto = item.quantidade * ing.currentPrice;
    ...
    try {
      ...
      console.log(`❌ Erro: ${receita.name} - ${error.message}`);
    }
  }
  
  console.log(`\n✅ ${atualizadas} receitas atualizadas!\n`);
  console.log('🔍 Acesse: http://localhost:3000/admin/cozinha/receitas');
}
